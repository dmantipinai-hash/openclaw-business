...
## 23. Красная кнопка (Kill Switch) — аварийная остановка всех агентов

### Зачем

Для любого enterprise-развёртывания критически важно иметь механизм **аварийной остановки**:

- агент начал вести себя непредсказуемо;
- ИБ-отдел получил alert и должен немедленно остановить работу;
- инцидент безопасности — нужно блокировать все сессии мгновенно;
- регуляторное требование — наличие «рубильника».

Аналоги: kill switch в банковских системах, военных, ядерной индустрии. Для агентской инфраструктуры — **обязателен**.

### Требования

1. **Одна кнопка** — остановить одного агента, нескольких или ВСЕХ
2. **Немедленно** — даже если агент уже получил ответ от LLM API
3. **Из другого процесса** — kill switch НЕ может быть частью OpenClaw Gateway (иначе если Gateway зависнет — кнопка не сработает)
4. **Для ИБ-отдела** — простой интерфейс, не CLI
5. **Из отдельной машины** — watchdog установлен на отдельном хосте

### Что OpenClaw УЖЕ умеет

| Уровень | Команда | Что делает |
|---------|---------|------------|
| Полная остановка | `openclaw gateway stop` | Убивает весь Gateway + ВСЕ агенты мгновенно |
| Docker | `docker compose down` | Убивает контейнер + все сессии |
| Systemd | `systemctl stop openclaw-gateway` | Системная остановка |
| Сетевой рубильник | Firewall block port 18789 | Gateway работает но недоступен |

### Целевая архитектура

```text
┌─────────────────────────────────────────────────┐
│  МАШИНА ИБ-ОТДЕЛА (отдельный хост)             │
│                                                  │
│  ┌──────────────────┐   ┌────────────────────┐  │
│  │  Kill Switch UI   │   │  Watchdog          │  │
│  │  (nginx + HTML)   │──▶│  (Python/Node)     │  │
│  │  порт 8080        │   │  имеет доступ к    │  │
│  │                   │   │  API / SSH         │  │
│  └──────────────────┘   └────────┬───────────┘  │
└──────────────────────────────────┼──────────────┘
                                   │
...
**Мягкая (Gateway API):**
- `POST http://<gateway>:18789/api/shutdown` — корректное завершение
- Работает если Gateway жив и отвечает
- Сохраняет state, завершает текущие операции

**Жёсткая (через хост):**
- `docker stop <container>` или `kill -9 <PID>`
- Работает всегда, даже если Gateway завис
- Данные текущей операции теряются

### Kill Switch должен уметь

| Действие | API / Метод |
|----------|-------------|
| Список всех агентов | `GET /api/agents` от каждого Gateway |
| Статус агента (alive/dead) | healthcheck каждые 10с |
| Активные сессии | `GET /api/sessions` |
| Крон-задачи | `GET /api/cron` |
| Убить одного агента | `POST /api/agents/{id}/kill` |
| Убить агента на конкретном Gateway | `docker stop <container>` или SSH kill |
| **УБИТЬ ВСЕХ** | Итерация по всем Gateway → остановить каждый |
| Перезапустить одного | `docker start <container>` |
| Журнал операций | Логировать все kill/restart действия с timestamp |

### Dashboard (UI)

Простой веб-интерфейс на HTML/CSS/JS (без фреймворков):

- Таблица всех Gateway-хостов с IP, порт, статус
- Для каждого Gateway: список агентов, сессий, cron-задач
- Кнопки: «Остановить агента», «Остановить Gateway», «ОСТАНОВИТЬ ВСЕХ»
- Красная кнопка «🛑 EMERGENCY KILL ALL» с подтверждением
- Журнал операций (кто, когда, что остановил)

### Размещение в репозитории

```
kill-switch/                  ← ОТДЕЛЬНАЯ ПАПКА
├── README.md                 ← инструкция установки (ОТДЕЛЬНАЯ МАШИНА)
├── docker-compose.yml        ← watchdog + dashboard контейнеры
├── dashboard/
│   └── index.html            ← простой веб-UI
└── watchdog/
    └── watchdog.py           ← логика kill/restart/status
```

**Важно:** Kill Switch устанавливается на **отдельную машину**, не туда же где OpenClaw Gateway. Это критическое требование безопасности.

### Инструкция по установке

См. `kill-switch/README.md` — пошаговая инструкция.

### Kill Switch — аварийная остановка всех агентов (РЕАЛИЗОВАНО ✅)

**Версия:** 1.0 | **Дата:** 2026-05-18 | **Статус:** готов к тестированию

См. `kill-switch/README.md` — полная документация, архитектура, инструкция установки.

**Что реализовано:**
- Watchdog (Python + Flask): опрос Gateway-хостов, мягкая/жёсткая остановка, логирование
- Dashboard (HTML/CSS/JS): статус, кнопки kill/restart, EMERGENCY KILL ALL
- Docker Compose: watchdog + nginx, запускается одной командой
- Установка на отдельную машину от Gateway

**Юнит-тесты ✅:**
- 27 тестов / 27 прошли (6.5s)
- Mock-объекты — реальный Gateway не нужен
- Покрытие: health check, soft/hard kill, kill all, restart, agents list, helpers
- Файлы: `kill-switch/watchdog/tests/test_watchdog.py`, `conftest.py`

### Инструкция по интеграции

**Шаг 1: Подготовка машины ИБ-отдела**
```bash
# Требования: Docker + Docker Compose, сетевой доступ к Gateway-хостам
# НЕ ставить на тот же хост что Gateway!
git clone <repo> /opt/kill-switch
cd /opt/kill-switch/kill-switch
cp gateways.json.example watchdog/gateways.json
```

**Шаг 2: Настроить gateways.json**
```json
[
  {
    "name": "gateway-prod-1",
    "host": "10.0.0.10",
    "port": 18789,
    "auth_token": "<token-from-gateway-config>"
  },
  {
    "name": "gateway-prod-2",
    "host": "10.0.0.11",
...
```
- `auth_token` берётся из конфига OpenClaw (`gateway.auth.token`)
- `host` — внутренний IP, НЕ публичный

**Шаг 3: Запуск**
```bash
docker compose up -d
# Dashboard: http://<ib-machine>:8080
# Watchdog API: http://<ib-machine>:5000
```

**Шаг 4: Проверка**
```bash
# Health check
watchdog API: http://<ib-machine>:5000/api/health

# Запуск тестов (на машине разработки)
cd kill-switch/watchdog
python -m pytest tests/ -v
```

**Шаг 5: Firewall**
- Порт 8080 доступен только с рабочих станций ИБ-отдела
- Порт 5000 (watchdog API) не торчит наружу
- Watchdog имеет доступ к портам 18789 на всех Gateway-хостах

### Ограничения прототипа (30%)

Это прототип для демонстрации концепции. Для production нужно:
- [ ] Реальное тестирование с живыми Gateway (несколько хостов)
- [ ] Валидация API endpoints (`/api/shutdown`, `/api/agents`, `/api/sessions`)
- [ ] TLS между watchdog и Gateway
- [ ] Авторизация на самом dashboard (basic auth / SSO)
- [ ] WebSocket для real-time обновления статуса
- [ ] Уведомления (email/Slack) при kill операциях
- [ ] Rate limiting на emergency kill

---

## 24. Изоляция инструментов (Tool Policy) — запрет внешних обращений

### Принцип

OpenClaw по умолчанию даёт агенту доступ к множеству инструментов: web-поиск, exec, браузер, файловая система. Для enterprise-контура это **неприемлемо** — агент не должен выходить за пределы разрешённого периметра.

**Правило: deny-all по умолчанию, whitelist только то что нужно.**

### Три профиля изоляции

#### Strict (комплаенс-зоны, конфиденциальные данные)

Разрешено только чтение и поиск по памяти. Никаких внешних обращений.

```yaml
# enterprise-strict.yaml
tools:
  profile: "deny-all"
  allow:
    - read            # чтение файлов (workspace-only)
    - memory_search   # поиск по памяти
    - memory_get      # чтение из памяти
    - write           # только если нужна запись в workspace
  deny:
    - web_search      # интернет-поиск — ЗАПРЕЩЁН
    - web_fetch       # загрузка URL — ЗАПРЕЩЕНА
    - exec            # выполнение команд — ЗАПРЕЩЕНО
    - browser         # браузер — ЗАПРЕЩЁН
    - image_generate  # генерация изображений — ЗАПРЕЩЕНА
    - sessions_spawn  # создание субагентов — ЗАПРЕЩЕНО
  fs:
    workspaceOnly: true   # файловые операции только внутри workspace
  exec:
    disabled: true        # exec полностью отключён
```

**Для кого:** отделы с конфиденциальными данными, юридический, финансовый.

#### Standard (инженерные команды, общие задачи)

Запрещены внешние обращения, но разрешена работа с кодом внутри workspace.
...
tools:
  ...
  deny:
    ...
    - web_fetch       # ЗАПРЕЩЁНА
    - browser         # ЗАПРЕЩЁН
    - sessions_spawn  # только с разрешения
  exec:
    applyPatch:
      workspaceOnly: true    # патчи только внутри workspace
  fs:
    workspaceOnly: true      # файлы только в workspace
  agents:
    defaults:
      sandbox:
        mode: "require"      # субагенты — только в sandbox
```

**Для кого:** разработчики, инженеры, техподдержка.

#### Sandboxed Exec (автоматизация, CI/CD)

Exec разрешён в песочнице. Внешние обращения — через allowlist.

```yaml
# enterprise-sandboxed.yaml
tools:
  profile: "deny-all"
  allow:
    - read
    ...
    - memory_search
    - memory_get
    - sessions_spawn   # делегация разрешена в sandbox
  deny:
    - web_search       # ЗАПРЕЩЁН (или заменить на внутренний)
    - web_fetch        # ЗАПРЕЩЁНА (кроме внутренних доменов)
    - browser
  web_fetch:
    allowedDomains:    # если нужен web_fetch — только эти домены
      - "internal.corp.local"
...
**Для кого:** CI/CD, автоматизация, интеграционные задачи.

### Как применить

1. Выбрать профиль из трёх выше (или создать свой)
2. Добавить в конфигурацию Gateway (`gateway.yaml` или через `openclaw config`)
3. Проверить: `openclaw security audit`
4. Убедиться что запрещённые инструменты действительно недоступны

### Готовый файл

См. `configs/enterprise-tool-profiles/` — три готовых YAML-файла с профилями.

### Что важно понимать

Это **обёртка-инструкция**. Конкретный набор инструментов зависит от предприятия:

- Какие внутренние сервисы нужны (GitLab, Jira, LDAP)
- Какие данные доступны агенту
- Какая политика ИБ принята

Мы даём **отправную точку и три шаблона**. Дальше — настройка под конкретного заказчика.

---

## 25. Compliance Logging — чёрный ящик для отдела комплаенс

### Принцип

Действия пользователя и агента должны логироваться и отправляться в **отдельный сервис**, недоступный пользователю. Пользователь **не может** удалить или изменить эти логи.

Это аналогично «чёрному ящику» в авиации — запись ведётся независимо от объекта наблюдения.

### Архитектура

```text
┌──────────────────────┐      ┌─────────────────────┐      ┌──────────────────────┐
│  OpenClaw Gateway    │      │  Log Shipper         │      │  Compliance Server   │
│  (машина агента)     │─────▶│  (та же машина,      │─────▶│  (ОТДЕЛЬНАЯ машина,  │
│                      │      │   работает как root)  │      │   отдел комплаенс)    │
│  ~/.openclaw/logs/   │      │                      │      │                      │
│  security-audit.json │      │  Читает → Фильтрует  │      │  Хранит → Индексирует│
│  sessions/*.jsonl    │      │  → Отправляет         │      │  → Предоставляет API  │
└──────────────────────┘      └─────────────────────┘      └──────────────────────┘
        ПОЛЬЗОВАТЕЛЬ                  ПОЛЬЗОВАТЕЛЬ                   КОМПЛАЕНС
     (не контролирует)           (не может отключить)          (полный контроль)
```
...
```json
{
  ...
  "user_id": "user@example.corp",
  "session_id": "abc-123",
  "agent_id": "main",
  "type": "tool_call",
  "tool": "exec",
  "input_summary": "ls -la /workspace",
  "output_summary": "5 files listed",
  "duration_ms": 120,
  "risk_level": "low"
}
```
...
**Типы событий:**
- `user_message` — что запросил пользователь
- `tool_call` — какой инструмент вызвал агент
- `tool_result` — результат выполнения
- `file_read` / `file_write` — работа с файлами
- `exec_command` — выполнение shell-команды
- `agent_spawn` — создание субагента
- `kill_operation` — остановка через Kill Switch
- `config_change` — изменение конфигурации

### Log Shipper (на стороне пользователя)

Это минимальный сервис, который:

1. Читает логи OpenClaw из `~/.openclaw/logs/`
2. Формирует записи в стандартном формате (JSON)
3. Отправляет batch-запросом на Compliance Server
4. Работает как **systemd** (Linux) или **launchd** (macOS) — пользователь не может отключить
5. При невозможности отправки — буферизирует локально

**Пример прототипа:** `compliance/log-shipper.py` — Python-скрипт, ~100 строк.

Запуск:
```bash
# Как systemd сервис (Linux)
sudo cp compliance/log-shipper.service /etc/systemd/system/
sudo systemctl enable --now log-shipper

# Как launchd агент (macOS)
sudo cp compliance/com.openclaw.log-shipper.plist /Library/LaunchDaemons/
sudo launchctl load /Library/LaunchDaemons/com.openclaw.log-shipper.plist
```

### Compliance Server (на стороне предприятия)

Мы **НЕ реализуем** этот сервер полностью. Это задача предприятия.

Мы определяем только **API-контракт**, который log-shipper ожидает:
...
```
GET /api/v1/audit?user=X&from=2026-05-01&to=2026-05-18
Authorization: Bearer <compliance-token>

→ 200 { "entries": [...] }
```

**Предприятие реализует хранение:**
- ELK Stack (Elasticsearch + Kibana)
- SIEM-система (Splunk, QRadar, Wazuh)
- Отдельная БД (PostgreSQL + отдельный API)
- Любой другой вариант, соответствующий политике ИБ

### Почему отдельный сервис

1. **Пользователь не может удалить логи** — они уже отправлены
2. **Пользователь не может подменить логи** — подпись или mTLS
3. **Комплаенс имеет полный контроль** — кто, когда, что делал
4. **Аналогия с Kill Switch** — логи живут на отдельной машине, как и кнопка аварийной остановки

### Что мы предоставляем

- `compliance/log-shipper.py` — прототип log shipper (~100 строк Python)
- `compliance/log-shipper.service` — systemd unit
- `compliance/com.openclaw.log-shipper.plist` — macOS launchd
- `compliance/audit-format.json` — схема формата логов
- Эту секцию документации

### Что делает предприятие

- Разворачивает Compliance Server (ELK/SIEM/своё решение)
- Реализует API-контракт (`POST /api/v1/audit/batch`)
- Настраивает TLS/mTLS между shipper и сервером
- Определяет политику хранения (сколько хранить, кто имеет доступ)
- Подключает свои инструменты анализа и расследования

---

## 26. Итоговый взгляд — что мы даём и чего нет

### Что есть в этом репозитории

| Компонент | Статус | Что это |
|-----------|--------|----------|
| Kill Switch | ✅ Прототип | Watchdog + Dashboard + тесты (27/27) |
| Tool Policy | ✅ Инструкция | 3 профиля изоляции + шаблоны YAML |
| Compliance Logging | ✅ Обёртка | Log shipper + формат + API-контракт |
| Документация | ✅ Полная | Этот файл — архитектура + инструкции |

### Чего НЕТ (и не будет без реального заказчика)

- Готового Compliance Server — это задача предприятия
- Интеграций с конкретными внутренними сервисами — нужны требования
- Production-тестирования — нужен реальный контур
- Полной RBAC/ABAC модели — нужна специфика заказчика

### Почему так

Без реального предприятия с конкретными требованиями невозможно доработать до конца. Максимум — 50% работы можно сделать «вслепую», и есть риск что всё окажется не в ту сторону.

Мы даём **прототип + инструкции + шаблоны**. Кто внимательно прочитает — разберётся и доработает под свою инфраструктуру.

---

*Обновлено: 2026-05-18 — Добавлены секции 24 (Tool Policy), 25 (Compliance Logging), 26 (Итог). Фича 9 завершена.*
